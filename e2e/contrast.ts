 ...
 *    files the node under `incomplete`. This page washes a gradient behind copy:
 *    a broken comparison column paints
 *    `linear-gradient(0deg, var(--bad-bg), transparent 60%)` over `--surface`,
 *    and the status badge and attacker text sit directly on that wash.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. A disabled `.btn` dims its
 *    whole subtree to `opacity: 0.45`.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The session-key
 *    and signature `.hex` boxes, the `.table-scroll` wrappers around the
 *    trade-off / fidelity / survival tables, and the `.deploy-scheme` code
 *    chips are all `overflow-x: auto` and clip their contents at 380px. Content
 ...
 *    ships no SVG `<text>` (its byte bars are `<div>` segments), so the branch
 *    below never fires here — it is kept intact and defensive so the helper
 *    stays correct if a diagram with labelled shapes is ever added: an
 *    ancestor-only walk would composite such a label onto the svg's own
 *    transparent background and report a ratio nothing on screen has, so for SVG
 *    text any earlier sibling shape whose box contains the text box is
 *    composited first.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * accent surfaces with `color-mix(in oklab, …)` (the hero "why" panel) and
     * `color-mix(in srgb, …)` (the highlighted "this lab" table rows and the
     * live survival row), and Chromium reports those to `getComputedStyle`
     * unchanged rather than converting them to sRGB. A regex that only
     * understands `rgb()/rgba()` sees `null` for every one of them and the walk
     * then falls through to the wrong backdrop — which fabricates failures and,
     * far worse, could hide a real one. The 2D canvas is the browser's own colour
     ...
     * that spans its element, but this lab's broken-column wash
     * (`linear-gradient(0deg, var(--bad-bg), transparent 60%)`) fades out over
     * the top 60% of the column, and the status text sits low in it — so judging
     * that text by the strongest `--bad-bg` stop would invent a tint it never
     * sits on. So each gradient is *sampled* at the text's real location instead:
     * linear by projecting onto the gradient line, radial by distance from the
     * centre over the farthest-corner radius (kept for gradients this lab does
     * not yet use). A non-gradient layer (`url()`, `none`) paints nothing.
     */
     ...
     * that here so a deliberately dimmed disabled control — the benchmark
     * button while a run is in flight — is not reported as a failure the spec
     * does not actually require fixing.
     */
     ...
     * it, the anatomy rows' and status badges' `aria-hidden` lock/shield emoji
     * (🔒/🔓/🛡️/⛔/✅) are measured as if each glyph took its element's declared
     * `color` — but an emoji paints in its own multicolour, so that ratio
    ...
    for (const el of Array.from(document.querySelectorAll('body *'))) {
      ...
      const cs = styleOf(el);
      // SVG text takes its ink from `fill`, not `color`, so where a page has
      // `<text>` its glyphs are painted in `fill` and reading `color` would
      // measure an inherited value the glyphs are not painted in. This lab ships
      // no SVG text today; the branch is kept correct for when one is added.
      ...
      // an ancestor's background — see the header note on the combiner diagram.
      let fg = fgRaw;
