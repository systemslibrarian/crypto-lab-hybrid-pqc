...
Establish a real session key three ways (classical / post-quantum / hybrid) and sign a message you type three ways, then flip the two threat switches — "quantum computer exists" and "lattices fall" — to watch every column recolor in real time. The attacker runs on every repaint whether or not anything is broken: they reconstruct a session key with the real combiner from whatever secrets they hold and a guess for the rest, and they forge a signature over a message of their choosing. When an approach is fully broken you see the reconstructed key match and the forgery pass verification; when only one half of a hybrid falls you see the same attack produce a key that does not match and a signature the verifier rejects, with the per-half rows showing which verifier said what. The "who survives which break" table under the switches is twelve of those runs, not a lookup table. Controls include per-approach and all-at-once buttons for key exchange and signing, an editable message for the signatures, and a trade-off matrix summarizing security, maturity, speed, bandwidth, and complexity.
